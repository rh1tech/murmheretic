...
{
    // Copy 320x200
    memcpy(dest_screen, raw, SCREENWIDTH * 200 * sizeof(*dest_screen));
    // Clear the rest (40 lines) if screen is taller
    if (SCREENHEIGHT > 200) {
        memset(dest_screen + SCREENWIDTH * 200, 0, SCREENWIDTH * (SCREENHEIGHT - 200) * sizeof(*dest_screen));
    }
}
