...
{
    visplanes = (visplane_t *)psram_malloc(MAXVISPLANES * sizeof(visplane_t));
    openings = (short *)psram_malloc(MAXOPENINGS * sizeof(short));
}
...
void __not_in_flash_func(R_MapPlane)(int y, int x1, int x2)
{
    angle_t angle;
...
void __not_in_flash_func(R_MakeSpans)(int x, int t1, int b1, int t2, int b2)
{
    while (t1 < t2 && t1 <= b1)
    {
...
void __not_in_flash_func(R_DrawPlanes)(void)
{
    visplane_t *pl;
    int light;
    int x, stop;
        ...
        if (pl->picnum == skyflatnum)
        {
            ...
            for (x = pl->minx; x <= pl->maxx; x++)
            {
                ...
                if (dc_yl <= dc_yh)
                {
                    ...
                    while (count >= 4)
                    {
                        *dest = dc_source[frac]; dest += SCREENWIDTH; frac += fracstep;
                        *dest = dc_source[frac]; dest += SCREENWIDTH; frac += fracstep;
                        *dest = dc_source[frac]; dest += SCREENWIDTH; frac += fracstep;
                        *dest = dc_source[frac]; dest += SCREENWIDTH; frac += fracstep;
                        count -= 4;
