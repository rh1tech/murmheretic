...
void SB_Drawer(void)
{
    ...
    else
    {
        if (SB_state == -1)
        {
            V_DrawPatch(0, ST_Y, PatchBARBACK);
            if (players[consoleplayer].cheats & CF_GODMODE)
            {
                V_DrawPatch(16, ST_Y + 9,
                            W_CacheLumpName(DEH_String("GOD1"), PU_CACHE));
                V_DrawPatch(287, ST_Y + 9,
        ...
        if (!inventory)
        {
            ...
            {
                // Main interface
                V_DrawPatch(34, ST_Y + 2, PatchSTATBAR);
                oldarti = 0;
                oldammo = -1;
                oldarmor = -1;
                oldweapon = -1;
        ...
        else
        {
            if (SB_state != 1)
            {
                V_DrawPatch(34, ST_Y + 2, PatchINVBAR);
            }
            DrawInventoryBar();
            SB_state = 1;
        }
...
void DrawCommonBar(void)
{
    ...
    V_DrawPatch(0, ST_Y - 10, PatchLTFCTOP);
    V_DrawPatch(290, ST_Y - 10, PatchRTFCTOP);
    ...
    if (oldhealth != HealthMarker)
    {
        ...
        chainY =
            (HealthMarker == CPlayer->mo->health) ? ST_Y + 33 : ST_Y + 33 + ChainWiggle;
        V_DrawPatch(0, ST_Y + 32, PatchCHAINBACK);
        V_DrawPatch(2 + (healthPos % 17), chainY, PatchCHAIN);
        V_DrawPatch(17 + healthPos, chainY, PatchLIFEGEM);
        V_DrawPatch(0, ST_Y + 32, PatchLTFACE);
        V_DrawPatch(276, ST_Y + 32, PatchRTFACE);
        ShadeChain();
...
void DrawMainBar(void)
{
    ...
    if (ArtifactFlash)
    {
        V_DrawPatch(180, ST_Y + 3, PatchBLACKSQ);

        temp = W_GetNumForName(DEH_String("useartia")) + ArtifactFlash - 1;

        V_DrawPatch(182, ST_Y + 3, W_CacheLumpNum(temp, PU_CACHE));
        ArtifactFlash--;
             ...
             || oldartiCount != CPlayer->inventory[inv_ptr].count)
    {
        ...
        {
            V_DrawPatch(179, ST_Y + 2,
                        W_CacheLumpName(DEH_String(patcharti[CPlayer->readyArtifact]),
                                        PU_CACHE));
            DrSmallNumber(CPlayer->inventory[inv_ptr].count, 201, ST_Y + 24);
        }
    ...
    if (deathmatch)
    {
        ...
        if (temp != oldfrags)
        {
            V_DrawPatch(57, ST_Y + 13, PatchARMCLEAR);
            DrINumber(temp, 61, ST_Y + 12);
            oldfrags = temp;
            UpdateState |= I_STATBAR;
        }
    }
    ...
    if (oldkeys != playerkeys)
    {
        ...
        {
            V_DrawPatch(153, ST_Y + 6, W_CacheLumpName(DEH_String("ykeyicon"), PU_CACHE));
        }
        if (CPlayer->keys[key_green])
        {
            V_DrawPatch(153, ST_Y + 14, W_CacheLumpName(DEH_String("gkeyicon"), PU_CACHE));
        }
        if (CPlayer->keys[key_blue])
        {
            V_DrawPatch(153, ST_Y + 22, W_CacheLumpName(DEH_String("bkeyicon"), PU_CACHE));
        }
        oldkeys = playerkeys;
        UpdateState |= I_STATBAR;
    }
    ...
    {
        V_DrawPatch(108, ST_Y + 3, PatchBLACKSQ);
        if (temp && CPlayer->readyweapon > 0 && CPlayer->readyweapon < 7)
        {
            DrINumber(temp, 109, ST_Y + 4);
            V_DrawPatch(111, ST_Y + 14,
    ...
    {
        V_DrawPatch(224, ST_Y + 13, PatchARMCLEAR);
        DrINumber(CPlayer->armorpoints, 228, ST_Y + 12);
...
void DrawInventoryBar(void)
{
    ...
    for (i = 0; i < 7; i++)
    {
            ...
            && CPlayer->inventory[x + i].type != arti_none)
        {
            ...
            V_DrawPatch(50 + i * 31, ST_Y + 2, W_CacheLumpName(patch, PU_CACHE));
            DrSmallNumber(CPlayer->inventory[x + i].count, 69 + i * 31, ST_Y + 24);
        }
    }
    V_DrawPatch(50 + curpos * 31, ST_Y + 31, PatchSELECTBOX);
    if (x != 0)
    {
        V_DrawPatch(38, ST_Y + 1, !(leveltime & 4) ? PatchINVLFGEM1 :
                    PatchINVLFGEM2);
    }
    if (CPlayer->inventorySlotNum - x > 7)
    {
        V_DrawPatch(269, ST_Y + 1, !(leveltime & 4) ?
...
void DrawFullScreenStuff(void)
{
    ...
    if (!inventory)
    {
        if (CPlayer->readyArtifact > 0)
        {
            ...
            V_DrawAltTLPatch(286, ST_Y + 12, W_CacheLumpName(DEH_String("ARTIBOX"), PU_CACHE));
            V_DrawPatch(286, ST_Y + 12, W_CacheLumpName(patch, PU_CACHE));
            DrSmallNumber(CPlayer->inventory[inv_ptr].count, 307, ST_Y + 34);
        }
    }
    else
    {
        ...
        for (i = 0; i < 7; i++)
        {
            V_DrawAltTLPatch(50 + i * 31, ST_Y + 10,
                          W_CacheLumpName(DEH_String("ARTIBOX"), PU_CACHE));
            if (CPlayer->inventorySlotNum > x + i
                && CPlayer->inventory[x + i].type != arti_none)
            {
                patch = DEH_String(patcharti[CPlayer->inventory[x + i].type]);
                V_DrawPatch(50 + i * 31, ST_Y + 10,
                            W_CacheLumpName(patch, PU_CACHE));
                DrSmallNumber(CPlayer->inventory[x + i].count, 69 + i * 31,
                              ST_Y + 32);
            }
        }
        V_DrawPatch(50 + curpos * 31, ST_Y + 39, PatchSELECTBOX);
        if (x != 0)
        {
            V_DrawPatch(38, ST_Y + 9, !(leveltime & 4) ? PatchINVLFGEM1 :
                        PatchINVLFGEM2);
        }
        if (CPlayer->inventorySlotNum - x > 7)
        {
            V_DrawPatch(269, ST_Y + 9, !(leveltime & 4) ?
                        PatchINVRTGEM1 : PatchINVRTGEM2);
        }
    }
}
