...
void TryRunTics (void)
{
    ...
    /*
    static int entry_logs = 0;
    if (entry_logs < 10) {
        printf("TryRunTics: Entering loop check. lowtic=%d, gametic=%d, counts=%d\n", lowtic, gametic, counts);
        entry_logs++;
    }
    */
    while (!PlayersInGame() || lowtic < gametic/ticdup + counts)
    {
        ...
        if (wait_log++ % 100 == 0) {
            printf("TryRunTics: Waiting... PlayersInGame=%d, lowtic=%d, gametic=%d, maketic=%d, time=%d, lasttime=%d\n", 
                   PlayersInGame(), lowtic, gametic, maketic, I_GetTimeMS(), lasttime);
        }
        */
